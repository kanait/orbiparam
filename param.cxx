...
//
//
// parameterization with free boundary
//
// $Id: $
//
...
int main(int argc, char **argv)
{
  ...
  if (argc != 7)
    {
      std::cerr << "Usage:  " << argv[0] << " infile outfile solver weight boundary save" << std::endl;
      std::cerr << "\tsolver    ... 1: SparseLU, 2: BICGSTAB" << std::endl;
      std::cerr << "\tweight    ... 1: Cotangent Weight, 2: Mean Value Weight" << std::endl;
      std::cerr << "\tboundary  ... 1: Fixed Rectangle Boundary, 2: Natural Boundary" << std::endl;
  ...
  bou = atoi(argv[5]);
  if ( bou == FIXED_BOUNDARY ) std::cout << "boundary: fixed rectangle boundary" << std::endl;
  else if ( bou == NATURAL_BOUNDARY ) std::cout << "boundary: natural boundary" << std::endl;
  sav = atoi(argv[6]);
  ...
  applyParam_FixedBoundary( mesh, paramx, paramy, sol, wei );
  if ( bou == NATURAL_BOUNDARY )
    {
      //for ( int i = 0; i < 3; ++i )
      applyParam_NaturalBoundary( mesh, paramx, paramy, sol, wei );
    }
